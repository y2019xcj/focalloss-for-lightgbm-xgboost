...
class ComplexLoss:
    ...
    def __init__(self,alpha = 0.75,gamma = 0.5,smoothing_value = 0.0,num_class = 3,tree_type = 'lgb'):
        if alpha <= 0 or alpha >=1:
            raise Exception("alpha must in (0,1) ")
        self.alpha = alpha
        if gamma < 0:
            raise Exception('gamma must bigger than 0')
        self.gamma = gamma
        ...
        if num_class < 2:
            raise Exception('num_class must bigger than 2')
